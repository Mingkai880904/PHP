<?php
 ...
 $result = mysqli_query
           ($conn,
            "SELECT ksu_std_name, ksu_std_grade, ksu_std_department, ksu_std_age 
			 FROM  ksu_std_table 
			 where not(ksu_std_department = '$dept')
			 order by ksu_std_grade desc, ksu_std_age desc");
			 
 echo "<table border='1'>
 <tr>
    <th>學生姓名 </th> <th>學生成績 </th> <th>年齡</th> <th>系別 </th>  <th> 備註 </th>
 </tr>";
 ...
 {	   
   ...
   echo "</tr>";
   $row_num = $row_num+1;
   $avg = $avg + $row['ksu_std_grade'];  	
 }
 echo "</table> <br/>";
 echo $row_num . " 名學生被查詢到!"."<br/>";
 echo " 其中不及格的人數"."$num_failed 人"."<br/>";
 echo " 其中高於79分的人數"."$num_highergrades 人"."<br/><br/>";
 echo " 上列學生總平均: " .sprintf ("%.3f",$avg/$row_num) ."分"; 
 echo "<br/>";
?> 
<form enctype="multipart/form-data"  method="post" action="midterm_ex41.html">
