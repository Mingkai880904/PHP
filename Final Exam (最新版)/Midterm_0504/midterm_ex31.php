<?php
 ...
 $new_std_name=str_replace("'","''",$_REQUEST['new_std_name']);
 ...
 echo "<table border='1'>
 <tr>
    <th>學生姓名 </th>  <th> 新的姓名 </th>  <th>學生成績 </th> <th> 備註 </th>
 </tr>";
 ...
 while($row = mysqli_fetch_array($result))
 {
   ...
   { 
      echo "<td style=\"background-color:yellow\">" . $row['ksu_std_name']  .   "</td>";
      echo "<td style=\"background-color:yellow\">" .str_replace($std_name, $new_std_name ,$row['ksu_std_name']) .  "</td>";
   ...
   {   
      ...
      echo "<td>" .str_replace($std_name, $new_std_name ,$row['ksu_std_name']) .  "</td>";
      echo "<td>" . $row['ksu_std_grade'] .   "</td>"; 
	  
	  if( $row['ksu_std_grade'] == 100 ){
         echo "<td style=\"color:blue\">" . "優" .   "</td>";
	  }
	  else {
		 echo "<td>" . "" .   "</td>";
	  }
...
?> 
<form enctype="multipart/form-data"  method="post" action="midterm_ex31.html">
